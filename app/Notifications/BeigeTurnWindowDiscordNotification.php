...
class BeigeTurnWindowDiscordNotification extends Notification
{
    use Queueable;

    /**
     * @param  EloquentCollection<int, Nation>  $nations
     */
    public function __construct(
        private readonly string $channelId,
        private readonly string $window,
        private readonly CarbonImmutable $turnChangeAt,
        private readonly EloquentCollection $nations
    ) {}
    ...
    public function toDiscordBot(object $notifiable): array
    {
        return [
            ...
            'payload' => [
                ...
                'window' => $this->window,
                'turn_change_at' => $this->turnChangeAt->toIso8601String(),
                'nation_count' => $this->nations->count(),
                'nations' => $this->nations->map(fn (Nation $nation): array => $this->formatNation($nation))->values()->all(),
            ],
        ];
    }
