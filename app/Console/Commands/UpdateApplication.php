...
class UpdateApplication extends Command
{
    protected $signature = 'app:update
        {--bypass : Run the legacy in-place update instead of deferring to the Nexus Setup updater}
        {--no-composer : Skip installing composer dependencies}
        {--no-node : Skip installing Node.js dependencies}';

    protected $description = 'Legacy in-place updater; use the Nexus Setup updater unless --bypass is passed';

    public function handle(): int
    {
        if (! $this->option('bypass')) {
            ...
            $this->line('To run the legacy in-place update anyway, pass --bypass.');

            return self::FAILURE;
        }

        $this->warn('Running the legacy in-place update because --bypass was passed.');
        Log::warning('Legacy application update started with --bypass.');
