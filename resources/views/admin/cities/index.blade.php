...
@section('content')
        ...
        <div class="w-full">
            <div class="row align-items-center">
                <div class="col-sm-7">
                    ...
                    <p class="text-base-content/50 mb-0">Live roster of every city in the alliance umbrella, including offshore partners.</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="row g-3">
        <div class="col-lg-3 col-sm-6">
            <x-admin.info-box icon="o-building-office-2" bgColor="badge-primary" title="Total Cities"
        ...
        <div class="col-lg-3 col-sm-6">
            @php
                $poweredPct = $summary['total_cities'] > 0
                    ...
                    : 0;
            @endphp
            <x-admin.info-box icon="o-bolt" bgColor="badge-success" title="Powered Cities"
                              :value="number_format($summary['powered_cities']) . ' (' . $poweredPct . '%)'"/>
        </div>
        <div class="col-lg-3 col-sm-6">
            <x-admin.info-box icon="o-arrow-trending-up" bgColor="badge-info" title="Avg Infrastructure"
                              :value="number_format($summary['average_infrastructure'], 2)"/>
        </div>
        <div class="col-lg-3 col-sm-6">
            <x-admin.info-box icon="o-map-pin" bgColor="badge-warning" title="Avg Land"
                              :value="number_format($summary['average_land'], 2)"/>
        </div>
    </div>

    <div class="card mt-4">
        <div class="card-header flex flex-column flex-lg-row gap-2 justify-content-between align-items-lg-center">
            <span class="font-semibold">Alliance Cities</span>
            <div class="flex flex-wrap gap-2 small">
                        ...
                        <tr @class(['bg-danger-subtle text-danger-emphasis' => ! $city->powered])>
                            <td>
                                <a href="https://politicsandwar.com/city/id={{ $city->id }}"
                                   target="_blank"
                                   rel="noopener"
                                   class="font-semibold link-underline-opacity-0 link-underline-opacity-75-hover">
                                    {{ $city->name }}
                                </a>
                            </td>
                            <td>
                            ...
                            <td>
                                @if($nation && $alliance)
                                    <a href="https://politicsandwar.com/alliance/id={{ $alliance->id }}"
                                       target="_blank"
                                       rel="noopener"
                                       class="badge badge-ghost link-underline-opacity-0 link-underline-opacity-75-hover">
                                        {{ $alliance->name }}
                                    </a>
                            ...
                            <td data-order="{{ $city->infrastructure }}"
                                @class(['bg-warning-subtle text-warning-emphasis font-semibold' => ! $infraAligned])
                            ...
                            <td data-order="{{ $city->land }}"
                                @class(['bg-warning-subtle text-warning-emphasis font-semibold' => ! $landAligned])
                                @if(! $landAligned) data-bs-toggle="tooltip" title="Land not divisible by 50" @endif>
                                {{ number_format($city->land, 2) }}
                            </td>
                            <td>
                                <span class="badge {{ $city->powered ? 'badge-success' : 'badge-error' }}">
