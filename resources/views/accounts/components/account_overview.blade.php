    ...
    <div class="overflow-x-auto">
        <table class="table w-full table-zebra">
            ...
            @foreach($accounts as $account)
                <tr class="hover">
                    <td>
                        <a href="{{ route("accounts.view", ["accounts" => $account]) }}" class="link link-primary">{{ $account->name }}</a>
                        <div class="tooltip" data-tip="Click to request a deposit">
                            <button class="deposit-request-btn relative" data-account-id="{{ $account->id }}">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                     stroke-width="1.5" stroke="currentColor" class="size-6 text-blue-500 hover:text-blue-700 cursor-pointer">
