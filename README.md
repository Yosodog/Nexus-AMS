...
Fresh databases created with `php artisan migrate --seed` initialize the tenant-private alliance setup guide. The first eligible local administrator sees a dismissible welcome prompt and can resume setup later from **Admin settings → Setup & readiness**; normal administration is never blocked. Installations upgraded without the setup metadata are treated as already complete until an administrator voluntarily starts the guide.

The guide reads readiness locally and never performs live Politics & War or Discord requests. Standalone operators own `PW_ALLIANCE_ID`, `PW_API_KEY`, `PW_API_MUTATION_KEY`, data synchronization, and any Discord connection configuration. In `hosted-tenant` mode, deployment credentials and Discord connection provisioning remain Nexus Cloud responsibilities, while local administrator security and alliance preferences remain tenant-local. The `world-writer` runtime cannot use the alliance setup flow. Setup status does not change Cloud route activation, bootstrap redemption, readiness responses, or infrastructure health.
